...
const BUILD_INCLUDE_DIRS: &'static [&'static str] = &[
    ...
    "libsel4/sel4_plat_include/$SEL4_PLATFORM$",
    "gen_config",
];
...
fn expand_include_dir(d: &str, arch: Arch, sel4_arch: SeL4Arch, ptr_width: usize) -> String {
    d.replace("$ARCH$", &arch.to_string())
        ...
        .replace("$SEL4_PLATFORM$", &format!("{}", env::var("SEL4_PLATFORM").expect("SEL4_PLATFORM unset")))
}
...
fn gen_bindings(
    ...
    let mut bindings = Builder::default()
        ...
        .use_core()
        .no_copy("seL4_IPCBuffer_")
        .no_copy("seL4_IPCBuffer")
    ...
    bindings = bindings.clang_arg("-nostdinc");
    ...
    Command::new("./fix_bindings.sh")
        .arg(out_file)
        .spawn()
        .expect("failed to fix bindings");
}
...
fn build_c_file(
    ...
    println!("cargo:rerun-if-changed=src/{}.c", base_name);

    let mut build = cc::Build::new();
    build.file(format!("src/{}.c", base_name));
    ...
    build.include(format!(
        ...
        kernel_path
            .join("include")
            .join("plat")
            .join(env::var("SEL4_PLATFORM").expect("SEL4_PLATFORM unset"))
            .display()
    ));
    build.include(format!("src/arch_include/{}", sel4_arch.to_string()));
    build.include("src");
