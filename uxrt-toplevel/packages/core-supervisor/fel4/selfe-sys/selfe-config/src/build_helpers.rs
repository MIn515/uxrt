...
pub fn print_config_real(){
    BuildEnv::request_reruns();
    let config = load_config_from_env_or_default();
    config.print_boolean_feature_flags();
}
pub fn print_config_dummy(){
    print!(r##"
cargo:rustc-cfg=HardwareDebugAPI
cargo:rustc-cfg=KernelColourPrinting
cargo:rustc-cfg=KernelDebugBuild
cargo:rustc-cfg=KernelFastpath
cargo:rustc-cfg=KernelIRQReporting
cargo:rustc-cfg=KernelIsMCS
cargo:rustc-cfg=KernelMultiboot1Header
cargo:rustc-cfg=KernelMultiboot2Header
cargo:rustc-cfg=KernelPrinting
cargo:rustc-cfg=KernelSkimWindow
"##);
}
