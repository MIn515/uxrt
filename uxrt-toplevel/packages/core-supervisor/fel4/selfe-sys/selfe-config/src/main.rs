...
fn build_kernel(
    ...
    if let Some(make_root_task_command) = root_task.make_command {
        ...
        let orig_rustflags = env::var("RUSTFLAGS").unwrap_or("".to_string());
        let rustflags = format!("{} -C link-arg=-T{}/cmake-tool/helpers/tls_rootserver.lds", orig_rustflags, tools_dir.display().to_string());
        ...
        if let Some(raw_target_specs) = config.build.target_specs.as_ref() {
            let target_specs = std::fs::canonicalize(raw_target_specs).expect("invalid target specifications directory").into_os_string().into_string().expect("invalid target specifications directory");
            build_cmd.env("RUST_TARGET_PATH", target_specs);
        }
...
fn symlink_build_dir(outcome: &SeL4BuildOutcome) {
    #[cfg(unix)]
    match outcome {
        SeL4BuildOutcome::StaticLib { .. } => {
            panic!("Should not be making a static lib when a kernel is expected")
        ...
        SeL4BuildOutcome::Kernel {
            ...
            //the result of removing an old symlink doesn't matter; if it 
            //couldn't be removed, creating a new one will panic
            let _ = std::fs::remove_file("./target/sel4-build");
            std::os::unix::fs::symlink(build_dir, "./target/sel4-build").expect("could not create symlink for build directory");
        }
    }
}
