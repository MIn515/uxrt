...
impl FromStr for Raw {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        fn parse_build(
            ...
            let mut target_specs = None;

            for (k, v) in table.iter() {
                if let Some(string) = v.as_str() {
                    if k.to_string() == String::from("target_specs_path") {
                        target_specs = Some(String::from(string));
                    }
                }
            }
            ...
            for (k, v) in table.iter() {
                if let Some(plat_table) = v.as_table() {
                    map.insert(k.to_string(), parse_platform_build(plat_table, target_specs.clone())?);
                } else if v.as_str().is_some() {
                    if k.to_string() != String::from("target_specs_path") {
                        panic!("unknown string key {} in build table", k.to_string());
                    }
                } else {
        ...
        fn parse_platform_build(table: &TomlTable, target_specs: Option<String>) -> Result<full::PlatformBuild, ImportError> {
