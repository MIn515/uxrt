...
impl Event {
    ...
    pub(super) fn with<F>(f: impl FnOnce(Pin<&Self>) -> F) -> F {
        ...
        let event = Self::new();
        event.thread.set(Some(UnparkHandle::current()));
        ...
        loop {
            ...
            match timeout {
                ...
                Some(_timeout) => {
                    ...
                    */
                    panic!("waiting with a timeout is currently unimplemented");
                }
            }
        }
    }
