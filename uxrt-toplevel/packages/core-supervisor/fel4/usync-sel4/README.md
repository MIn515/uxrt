...
This version has been ported to a `no_std` seL4 environment using the UX/RT feL4libraries. Instead of `std::thread`, `sel4-thread-park` is used for parking 
threads (the API semantics are equivalent). Timeouts are currently not supported
since seL4 provides no API for timeouts by itself.
