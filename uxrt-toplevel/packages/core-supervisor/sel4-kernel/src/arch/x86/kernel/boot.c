...
)
{
    ...
    if (mbi2->mbi_vaddr){
        extra_bi_size += sizeof(seL4_BootInfo_MBI2);
    }
    ...
    /* populate MBI2 pass-through */
    if (mbi2 && mbi2->mbi_vaddr) {
        mbi2->header.id = SEL4_BOOTINFO_HEADER_MBI2;  
        mbi2->header.len = sizeof(seL4_BootInfo_MBI2);
        memcpy((void*)(rootserver.extra_bi + extra_bi_offset), mbi2, sizeof(*mbi2));
        extra_bi_offset += sizeof(seL4_BootInfo_MBI2);
    }
