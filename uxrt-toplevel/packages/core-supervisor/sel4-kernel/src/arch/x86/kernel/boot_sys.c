...
#define EXEC_MODULE_ADDRESS_SIZE 8
#else
#define EXEC_MODULE_ADDRESS_SIZE 4
#endif

BOOT_CODE static paddr_t
load_boot_module_preloaded(const multiboot2_module_exec_t *root_task, multiboot2_header_t *mbi)
{
    v_region_t v_reg;
    int i;
    paddr_t load_paddr;
    multiboot2_module_sections_t *sections_tag = (multiboot2_module_sections_t *)(((word_t)root_task->sections) + sizeof(multiboot2_tag_t));
    if (sections_tag->address_size != EXEC_MODULE_ADDRESS_SIZE){
        printf("Root task has unsupported address size %d\n", sections_tag->address_size);    
        return 0;
    }

    for (i = 1; i < sections_tag->num_sections; i++){
        struct multiboot2_module_section *section = &(sections_tag->sections[i]);
        struct multiboot2_module_section *prev_section = &(sections_tag->sections[i-1]);
        if (section->mem_start < prev_section->mem_end){
            printf("Root task section at 0x%lx begins before preceding section ending at %lx\n", section->mem_start, prev_section->mem_end);
            return 0;
        }
    }

    v_reg.start = sections_tag->sections[0].mem_start;
    load_paddr = sections_tag->sections[0].file_start;
    v_reg.end = v_reg.start + boot_state.mods_end_paddr - root_task->start;


    if ((word_t)mbi < boot_state.mods_end_paddr){
        printf("Multiboot info does not follow boot modules: 0x%lx; end: 0x%lx, aligned: 0x%lx)\n", (long)mbi, (long)boot_state.mods_end_paddr, (long)ROUND_UP(boot_state.mods_end_paddr, PAGE_BITS));
        return (0);
    }

    v_reg.end += (word_t)mbi - boot_state.mods_end_paddr;
    v_reg.end += mbi->total_size;
    v_reg.end = ROUND_UP(v_reg.end, PAGE_BITS);

    printf("MBI: start: 0x%lx, aligned end: 0x%lx\n", (word_t)mbi, v_reg.end);
    ...
    load_paddr += v_reg.end - v_reg.start;
    boot_state.ui_info.p_reg.end = load_paddr;
    boot_state.ui_info.v_entry = sections_tag->entry;

    boot_state.mbi2_pt.mbi_vaddr = boot_state.mbi2_pt.mbi_paddr - boot_state.ui_info.pv_offset;

    printf("size=0x%lx v_entry=%p v_start=%p v_end=%p p_start=%p p_end=%p\n",
           v_reg.end - v_reg.start,
           (void*)sections_tag->entry,
           (void*)v_reg.start,
           (void*)v_reg.end,
           (void*)(v_reg.start + boot_state.ui_info.pv_offset),
           (void*)(v_reg.end + boot_state.ui_info.pv_offset)
          );

    if (!IS_ALIGNED(v_reg.start, PAGE_BITS)) {
        printf("Root task virtual start address %llx not 4KB-aligned\n", (unsigned long long)v_reg.start);
        return 0;
    }

    if ((sections_tag->entry < v_reg.start) || (sections_tag->entry >= v_reg.end)) {
        printf("Root task entry point 0x%llx outside root task image\n", (unsigned long long)sections_tag->entry);
        return 0;
    }
...
static BOOT_CODE bool_t try_boot_sys(void)
{
    ...
    if (boot_state.preloaded_root_task){
        printf("Setting up root task memory regions:\n");
        load_paddr = load_boot_module_preloaded(boot_state.preloaded_root_task, (multiboot2_header_t *)(boot_state.mbi2_pt.mbi_paddr));
    }else{
...
)
{
    ...
    boot_state.vbe_info.vbeMode = -1;
    boot_state.mbi2_pt.mbi_paddr = (word_t)mbi2;
    ...
    while (tag < tag_e && tag->type != MULTIBOOT2_TAG_END) {
        ...
        if (tag->type == MULTIBOOT2_TAG_CMDLINE) {
            ...
            acpi_tag_found = true;
        } else if (tag->type == MULTIBOOT2_TAG_MODULE || tag->type == MULTIBOOT2_TAG_MODULE_EXEC) {
            multiboot2_module_t const *module = (multiboot2_module_t const *)behind_tag;
            const char *preloaded = "";
            const char *cmdline;
            if (tag->type == MULTIBOOT2_TAG_MODULE_EXEC){
                cmdline = ((multiboot2_module_exec_t const *)module)->string;
                preloaded = " (preloaded)";
            }else{
                cmdline = ((multiboot2_module_t const *)module)->string;
            }

            printf(
                "  module #%d%s: start=0x%x end=0x%x size=0x%x name='%s'\n",
                mod_count,
                preloaded,
                module->start,
                module->end,
            ...
            if (tag->type == MULTIBOOT2_TAG_MODULE_EXEC){
                if (mod_count){
                    printf("Pre-loaded root task is not the first module\n");
                    return false;
                }
                if (boot_state.preloaded_root_task){
                    printf("Multiple pre-loaded executable modules present (only the root task should be pre-loaded)\n");
                    return false;
                }

                boot_state.preloaded_root_task = (multiboot2_module_exec_t *)module;
            }
        ...
        } else if (tag->type == MULTIBOOT2_TAG_MODULE_IMAGE) {
            multiboot2_module_t const * module = (multiboot2_module_t const *)behind_tag;
            printf(
                "  FS image: start=0x%x end=0x%x size=0x%x cmdline='%s'\n",
                module->start,
                module->end,
                module->end - module->start,
                (char *)module->string
            );
            boot_state.fs_image_found = true;
    ...
    if (boot_state.fs_image_found){
        if (!boot_state.preloaded_root_task){
            printf("FS image loaded without preloaded root task\n");
            return (false);
        }
        boot_state.mbi2_pt.mbi_paddr = (word_t)mbi2;
    }
    ...
    /* If no ACPI tag was present in the MBI, try to get the RSDP from the BIOS
     * instead */
    if (!acpi_tag_found && !acpi_init(&boot_state.acpi_rsdp)) {
        return false;
    }
    ...
    void *mbi)
{
    ...
    boot_state.fs_image_found = false;
