...
const TAG_RSDPV1: u32 = 14;
const TAG_RSDPV2: u32 = 15;

const TAG_XHI_MODULE_EXEC: u32 = 0x93;
const TAG_XHI_MODULE_IMAGE: u32 = 0xb3;
...
pub use module::{ModuleTag, ExecModuleTag, BaseModuleTag, ModuleIter, ExecModuleIter};
...
impl BootInformation {
    ...
    pub fn elf_sections_tag(&self) -> Option<ElfSectionsTag> {
        self.get_tag(TAG_ELF_SECTIONS).map(|tag| elf_sections::elf_sections_tag(tag))
    }

    pub fn memory_map_tag(&self) -> Option<&'static MemoryMapTag> {
        self.get_tag(TAG_MMAP).map(|tag| unsafe { &*(tag as *const Tag as *const MemoryMapTag) })
    }
    ...
    pub fn fs_image_module_tags(&self) -> ModuleIter {
        module::module_iter_fs_image(self.tags())
    }

    pub fn exec_module_tags(&self) -> ExecModuleIter {
        module::module_iter_exec(self.tags())
    }


    pub fn boot_loader_name_tag(&self) -> Option<&'static BootLoaderNameTag> {
        self.get_tag(TAG_BOOT_LOADER_NAME).map(|tag| unsafe { &*(tag as *const Tag as *const BootLoaderNameTag) })
    }

    pub fn command_line_tag(&self) -> Option<&'static CommandLineTag> {
        self.get_tag(TAG_CMDLINE).map(|tag| unsafe { &*(tag as *const Tag as *const CommandLineTag) })
    }

    pub fn framebuffer_tag(&self) -> Option<FramebufferTag<'static>> {
        self.get_tag(TAG_FRAMEBUFFER).map(|tag| framebuffer::framebuffer_tag(tag))
    }

    pub fn rsdp_v1_tag(&self) -> Option<&'static RsdpV1Tag> {
        self.get_tag(TAG_RSDPV1).map(|tag| unsafe { &*(tag as *const Tag as *const RsdpV1Tag) })
    }

    pub fn rsdp_v2_tag(&self) -> Option<&'static RsdpV2Tag> {
        self.get_tag(TAG_RSDPV2).map(|tag| unsafe { &*(tag as *const Tag as *const RsdpV2Tag) })
    }
